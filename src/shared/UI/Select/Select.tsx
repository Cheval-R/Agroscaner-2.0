...
const FindActiveOptionLabel = (optionsList: Identifiers[], activeKey: string): string => {
...
const Select = ({ optionsList, activeKey, legend, selectOption, onBlur, errorMessage }: Props) => {
  const activeKeyRef = useRef(activeKey);
  const activeOptionLabel: string = FindActiveOptionLabel(optionsList, activeKey);

  const [query, setQuery] = useState(activeOptionLabel);

  const { isOpen, toClose, toOpen, wrapperRef } = usePopover(() => {
    setQuery(activeKeyRef.current ? FindActiveOptionLabel(optionsList, activeKeyRef.current) : '');
  });
  useEffect(() => {
  ...
  const filteredOptions =
    clearQuery.length > 0
      ? optionsList.filter((option) => option.label.toLowerCase().includes(clearQuery))
      : null;
