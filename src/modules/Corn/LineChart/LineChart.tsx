...
import type { TForecastTemperature } from "../hook/usePredictionTemperature";
import type { ChartDataset } from "chart.js";
import { useSumEffectiveTemperature } from "../hook/useSumEffectiveTemperature";
...
interface IOptimalPolygon {
  startIndex: number;
  endIndex: number;
}

function createPolygonDataset(
  sumEffectiveTemperature: number[],
): ChartDataset<"line"> | undefined {
  const startIndex = sumEffectiveTemperature.findIndex(
    (temperature) => temperature >= 850,
  );
  let endIndex = sumEffectiveTemperature.findIndex(
    (temperature) => temperature >= 950,
  );

  if (startIndex === -1) return;
  if (endIndex === -1)
    endIndex = sumEffectiveTemperature[sumEffectiveTemperature.length - 1];
...
const LineChart: React.FC<Props> = ({ data, baseTemp }) => {
  const sumEffectiveTemperature = useSumEffectiveTemperature(data, baseTemp);
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        // Не показывать тултип для определённых датасетов
